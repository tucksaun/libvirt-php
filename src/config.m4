...
if test "$PHP_LIBXML" != "no"; then
  ...
  if test -z "$LIBXML_DIR"; then
    ...
    AC_MSG_ERROR(Please reinstall the libxml2 distribution - xmlstring.h should
                 be in <libxml2-dir>/libxml)
  fi
  ...
  if test -z "$LIBVIRT_DIR"; then
    AC_MSG_RESULT(not found)
    AC_MSG_ERROR(Please reinstall the libvirt distribution - libvirt.h should
                 be in <libvirt-dir>/include and libvirt.h should be in <libvirt-dir>/lib)
  fi
